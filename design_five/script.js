...
const responses = [
  "Agni Group is the exclusive distributor of Mahindra vehicles in Nepal. We have over 80 dealers nationwide!",
  "Looking for the XUV700? It's our flagship premium SUV with segment-best features. You can book a test drive easily.",
  "Our EV journey begins in 2025 with the Mahindra BE6 and XEV9e. Stay tuned!",
  "Balaju Auto Works handles our premium after-sales service to keep your vehicle in top condition.",
  "Agni Logistics provides complete supply chain management across Nepal."
];
...
function handleAiResponse() {
  ...
  if (query.includes('ev') || query.includes('electric')) {
    reply = responses[2];
  } else if (query.includes('service') || query.includes('repair')) {
    reply = responses[3];
  } else if (query.includes('xuv')) {
    reply = responses[1];
  }
  
  aiResponse.style.display = 'block';
  aiResponse.innerHTML = `<div style="margin-top: 1rem; padding: 1rem; background: rgba(249,115,22,0.1); border-radius: 12px; border-left: 4px solid var(--accent); color: var(--text-main); font-size: 0.95rem;"><strong>✦ Agni AI:</strong> ${reply}</div>`;
  aiInput.value = '';
}

aiSend.addEventListener('click', handleAiResponse);
